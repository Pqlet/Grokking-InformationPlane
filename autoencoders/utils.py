...
def plot_MI_planes(MI_X_L: dict, MI_L_Y: dict,
                   figure_path,
                   filtered_MI_X_L: dict=None, filtered_MI_L_Y: dict=None, log_steps=None,
    ...
    for index, layer_name in enumerate(MI_X_L.keys()):
        ...
        subplot_ax.set_title(str(layer_name), fontsize=26)
        ...
        if filtered_provided:
            ...
            # subplot_ax.plot(x, y)
            # scatter_iplane = subplot_ax.scatter(x, y, c=list(range(len(x))))
            scatter_iplane = subplot_ax.scatter(x, y, c=log_steps)
        
        # pqlet: produce a legend with the unique colors from the scatter
        legend1 = subplot_ax.legend(
            *scatter_iplane.legend_elements(), 
            loc="lower right", title="Steps", fontsize=12
        )
        subplot_ax.add_artist(legend1)
...
def get_layers(model, dataloader, device, apply_agn=True) -> list():
    """
    ...
    with torch.no_grad():
        for index, batch in enumerate(dataloader):
            ...
            layers = model(x.to(device), all_layers=True, apply_agn=apply_agn)
...
def save_results(results: dict, settings: dict, path: Path):
    ...
    for layer_name in results["MI_X_L"].keys():
        ...
        if results["filtered_MI_X_L"] is not None:
            MI_dataframe["filtered I(X;L)"] = results["filtered_MI_X_L"][layer_name]
        if results["filtered_MI_L_Y"] is not None:
